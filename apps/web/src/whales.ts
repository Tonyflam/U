 ...
 * built at deploy time, then a small client script refreshes them every 30s
 * by polling `/api/whales.json` (regenerated by the build).
 *
...
(function(){
  ...
  function refresh(){
    ...
    fetch('/api/whales.json?t='+Date.now()).then(function(r){return r.json()}).then(function(j){
      if(!j||!j.whales)return;
      dataAt=j.generatedAt||Date.now();
      document.body.setAttribute('data-generated-at',String(dataAt));
      j.whales.forEach(function(w){
        var card=document.querySelector('.wcard[data-address="'+w.address.toLowerCase()+'"]');
        if(!card)return;
        if(w.stale){card.querySelector('.wcard-stale')&&(card.querySelector('.wcard-stale').style.display='inline-flex');return}
        ...
        if(cells[0]){cells[0].textContent=fmtPnlSigned(w.thirtyDayUsd);cells[0].className='sv '+pnlClass(w.thirtyDayUsd)}
        if(cells[1]){cells[1].textContent=fmtPnlSigned(w.allTimeUsd);cells[1].className='sv '+pnlClass(w.allTimeUsd)}
        if(cells[2]){cells[2].textContent=fmtUsd(w.equityUsd);cells[2].className='sv'}
        var posWrap=card.querySelector('.pos-content');
        if(posWrap)posWrap.innerHTML=renderPositions(w.positions);
        var posCount=card.querySelector('.pos-count');
        if(posCount)posCount.textContent=(w.positions&&w.positions.length)?String(w.positions.length)+' open':'flat';
      });
